		...
		case EVENT_COMMIT:
			...
			DeleteGraphPlot(panel, PANEL_FREQ_GRAPH, -1, VAL_IMMEDIATE_DRAW);
				...
				if(type==2)
				{
					...
					//2-The polynomial order
					//27- nSidePoints int (2*nSidePoints +1) is frameSize, which specifies how many data points to use in the polynomial fitting. 
					//NULL- weightingVector
				}
